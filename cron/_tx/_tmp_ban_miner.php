...
do {
	...
	$num = $db_admin->query( __FILE__, __LINE__,  __FUNCTION__,  __CLASS__, __METHOD__, "
			SELECT count(*)
	...
	$res = $db_admin->query( __FILE__, __LINE__,  __FUNCTION__,  __CLASS__, __METHOD__, "
			SELECT `".DB_PREFIX."abuses`.`user_id`
			FROM `".DB_PREFIX."abuses`
			LEFT JOIN `".DB_PREFIX."miners_data` ON `".DB_PREFIX."miners_data`.`user_id` = `".DB_PREFIX."abuses`.`user_id`
			WHERE `miner_id` > 0
			GROUP BY `user_id`
			");
	...
	$type = ParseData::findType('admin_ban_miners');
	$time = time();
	$user_id = 1;
