...
do {
	...
	$type = ParseData::findType('admin_message');
	$time = time();
	$user_id =  1;
	$message = array();
	$message['gen'] = 'Hello World';
	$message['en'] = 'Hello World';
	$message['ru'] = 'Добро пожаловать!';
	$message = json_encode($message);
	$currency_list = 'ALL';
	$stend_id = 1;

	$data_for_sign = "{$type},{$time},{$user_id},{$message},{$currency_list}";
	...
	$data = dec_binary ($type, 1) .
		...
		ParseData::encode_length_plus_data($currency_list) .
