...
do {
	...
	foreach ($db_stends as $user_id=>$data) {
        ...
        while ( $row = $db->fetchArray( $res ) ) {
	        $commission_array[$row['id']][0] = 0.1;
	        $commission_array[$row['id']][1] = 0;
	        $commission_array[$row['id']][2] = 0;
        }
		$commission = json_encode($commission_array);

		$type = ParseData::findType('change_commission');
		$time = time();

        $data_for_sign = "{$type},{$time},{$user_id},{$commission}";
