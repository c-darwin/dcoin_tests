...
do {
	...
	while ($row = $db_admin->fetchArray($res))
		$abuses[$row['user_id']] = 'abuse '.rand(0, 9999999);
	if ($abuses) {

		$abuses = json_encode($abuses);
		$type = ParseData::findType('abuses');
		$time = time();
		$user_id = 1;
		$data_for_sign = "{$type},{$time},{$user_id},{$abuses}";
		...
		$data = dec_binary ($type, 1) .
			...
			ParseData::encode_length_plus_data($abuses) .
